        ...
        <>
            {window.innerWidth > 900
                ? < div className={styles['tb-orders']} >
                    ...
                    {user?.role !== 5051 && <button className={styles['button-53']} onClick={() => deleteItemHandler(order)}>-</button>}

                    {user?.role !== 5051 && <button className={styles['button-53-green']} onClick={() => addItemHandler(order)}>+</button>}
                    <div className={styles['ord-count']}>
                        <span className={styles['ord-counter']}>{order.count} </span> x {order.price.toFixed(2)}
                    </div>

                    {(order.family === 'food' && user?.role !== 5051) && <button className={styles['button-53-blue']} onClick={toChefHandler}> {sent} / {count}</button>}
